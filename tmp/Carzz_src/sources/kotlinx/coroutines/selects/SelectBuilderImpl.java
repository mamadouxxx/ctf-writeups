...
public final class SelectBuilderImpl<R> extends LockFreeLinkedListHead implements SelectBuilder<R>, SelectInstance<R>, Continuation<R>, CoroutineStackFrame {
    ...
    public final void handleBuilderException(Throwable e) {
        if (trySelect()) {
            ...
            resumeWith(Result.m142constructorimpl(ResultKt.createFailure(e)));
        } else {
            ...
            if (obj == SelectKt.UNDECIDED) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_result$FU, this, IntrinsicsKt.getCOROUTINE_SUSPENDED(), SelectKt.RESUMED)) {
                    if (Result.m148isFailureimpl(result)) {
                        Continuation<R> continuation = this.uCont;
                        Throwable thM145exceptionOrNullimpl = Result.m145exceptionOrNullimpl(result);
                        Intrinsics.checkNotNull(thM145exceptionOrNullimpl);
                        Result.Companion companion = Result.INSTANCE;
                        if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
                            thM145exceptionOrNullimpl = StackTraceRecoveryKt.recoverFromStackFrame(thM145exceptionOrNullimpl, (CoroutineStackFrame) continuation);
                        }
                        continuation.resumeWith(Result.m142constructorimpl(ResultKt.createFailure(thM145exceptionOrNullimpl)));
                        return;
                    }
        ...
        while (true) {
            ...
            if (obj == SelectKt.UNDECIDED) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_result$FU, this, IntrinsicsKt.getCOROUTINE_SUSPENDED(), SelectKt.RESUMED)) {
                    ...
                    Result.Companion companion = Result.INSTANCE;
                    continuationIntercepted.resumeWith(Result.m142constructorimpl(ResultKt.createFailure(exception)));
                    return;
                }
            } else {
                throw new IllegalStateException("Already resumed");
