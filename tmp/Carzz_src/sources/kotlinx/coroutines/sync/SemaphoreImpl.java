...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM1697constructorimpl;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1697constructorimpl = SegmentOrClosed.m1697constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1697constructorimpl = SegmentOrClosed.m1697constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1702isClosedimpl(objM1697constructorimpl)) {
                Segment segmentM1700getSegmentimpl = SegmentOrClosed.m1700getSegmentimpl(objM1697constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= segmentM1700getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!segmentM1700getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, segmentM1700getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (segmentM1700getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1700getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1700getSegmentimpl(objM1697constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, segmentM1700getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM1700getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
