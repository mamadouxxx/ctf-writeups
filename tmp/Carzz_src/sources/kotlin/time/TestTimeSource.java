...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1604plusAssignLRDsOJo(long duration) {
        long j;
        long jM1515toLongimpl = Duration.m1515toLongimpl(duration, getUnit());
        if (jM1515toLongimpl != Long.MIN_VALUE && jM1515toLongimpl != Long.MAX_VALUE) {
            long j2 = this.reading;
            j = j2 + jM1515toLongimpl;
            if ((jM1515toLongimpl ^ j2) >= 0 && (j2 ^ j) < 0) {
                m1603overflowLRDsOJo(duration);
            }
        } else {
            double dM1512toDoubleimpl = this.reading + Duration.m1512toDoubleimpl(duration, getUnit());
            if (dM1512toDoubleimpl > 9.223372036854776E18d || dM1512toDoubleimpl < -9.223372036854776E18d) {
                m1603overflowLRDsOJo(duration);
            }
            j = (long) dM1512toDoubleimpl;
        }
        this.reading = j;
    }
    ...
    private final void m1603overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1518toStringimpl(duration)) + '.');
    }
}
