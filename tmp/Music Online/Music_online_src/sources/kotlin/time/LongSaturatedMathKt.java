...
public final class LongSaturatedMathKt {
    ...
    public static final long m1629saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM1546toLongimpl = Duration.m1546toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m1628checkInfiniteSumDefinedPjuGub4(j, j2, jM1546toLongimpl);
        }
        if ((1 | (jM1546toLongimpl - 1)) == Long.MAX_VALUE) {
            return m1630saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM1546toLongimpl;
        return ((j ^ j3) & (jM1546toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m1628checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1532isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m1630saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM1503divUwyO8pc = Duration.m1503divUwyO8pc(j2, 2);
        long jM1546toLongimpl = Duration.m1546toLongimpl(jM1503divUwyO8pc, durationUnit);
        return (1 | (jM1546toLongimpl - 1)) == Long.MAX_VALUE ? jM1546toLongimpl : m1629saturatingAddNuflL3o(m1629saturatingAddNuflL3o(j, durationUnit, jM1503divUwyO8pc), durationUnit, Duration.m1535minusLRDsOJo(j2, jM1503divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m1602getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m1601getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m1553unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m1536plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m1553unaryMinusUwyO8pc(infinityOfSign(j3));
        }
