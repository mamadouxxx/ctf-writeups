...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    */
    ...
    public final Object mo1680trySendJP2dKIU(E element) {
        Object objOfferInternal = offerInternal(element);
        if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1700successJP2dKIU(Unit.INSTANCE);
        }
        if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1699failurePtdJZtk() : ChannelResult.INSTANCE.m1698closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (objOfferInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1698closedJP2dKIU(helpCloseAndGetSendException((Closed) objOfferInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (undeliveredElementExceptionCallUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m174constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
        Result.Companion companion2 = Result.INSTANCE;
        continuation.resumeWith(Result.m174constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1725remove()) {
                receive.helpRemove();
            } else {
                objM1717constructorimpl$default = InlineList.m1722plusFjFbRPM(objM1717constructorimpl$default, receive);
            }
        }
        if (objM1717constructorimpl$default != null) {
            if (!(objM1717constructorimpl$default instanceof ArrayList)) {
                ((Receive) objM1717constructorimpl$default).resumeReceiveClosed(closed);
            } else if (objM1717constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) objM1717constructorimpl$default;
