...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM1729constructorimpl;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1729constructorimpl = SegmentOrClosed.m1729constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1729constructorimpl = SegmentOrClosed.m1729constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1734isClosedimpl(objM1729constructorimpl)) {
                Segment segmentM1732getSegmentimpl = SegmentOrClosed.m1732getSegmentimpl(objM1729constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= segmentM1732getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!segmentM1732getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, segmentM1732getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (segmentM1732getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1732getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1732getSegmentimpl(objM1729constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, segmentM1732getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM1732getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
