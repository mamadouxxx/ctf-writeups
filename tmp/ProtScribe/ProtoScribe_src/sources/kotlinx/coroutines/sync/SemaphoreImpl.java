...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1772constructorimpl = SegmentOrClosed.m1772constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1772constructorimpl = SegmentOrClosed.m1772constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1777isClosedimpl(objM1772constructorimpl)) {
                Segment segmentM1775getSegmentimpl = SegmentOrClosed.m1775getSegmentimpl(objM1772constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= segmentM1775getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!segmentM1775getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, segmentM1775getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (segmentM1775getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1775getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1775getSegmentimpl(objM1772constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, segmentM1775getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (segmentM1775getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
