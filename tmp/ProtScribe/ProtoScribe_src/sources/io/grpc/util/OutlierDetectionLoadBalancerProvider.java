...
public final class OutlierDetectionLoadBalancerProvider extends LoadBalancerProvider {
    ...
    private NameResolver.ConfigOrError parseLoadBalancingPolicyConfigInternal(Map<String, ?> map) {
        ...
        NameResolver.ConfigOrError loadBalancingPolicyConfig = GracefulSwitchLoadBalancer.parseLoadBalancingPolicyConfig(JsonUtil.getListOfObjects(map, "childPolicy"));
        if (loadBalancingPolicyConfig.getError() != null) {
            return NameResolver.ConfigOrError.fromError(Status.INTERNAL.withDescription("Failed to parse child in outlier_detection_experimental: " + map).withCause(loadBalancingPolicyConfig.getError().asRuntimeException()));
        }
        builder.setChildConfig(loadBalancingPolicyConfig.getConfig());
