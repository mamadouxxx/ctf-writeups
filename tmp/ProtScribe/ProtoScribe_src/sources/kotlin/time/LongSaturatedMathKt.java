...
public final class LongSaturatedMathKt {
    ...
    public static final long m1672saturatingAddpTJri5U(long j, long j2) {
        long jM1564getInWholeNanosecondsimpl = Duration.m1564getInWholeNanosecondsimpl(j2);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m1671checkInfiniteSumDefinedPjuGub4(j, j2, jM1564getInWholeNanosecondsimpl);
        }
        if ((1 | (jM1564getInWholeNanosecondsimpl - 1)) == Long.MAX_VALUE) {
            return m1673saturatingAddInHalvespTJri5U(j, j2);
        }
        long j3 = j + jM1564getInWholeNanosecondsimpl;
        return ((j ^ j3) & (jM1564getInWholeNanosecondsimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m1671checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1576isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m1673saturatingAddInHalvespTJri5U(long j, long j2) {
        long jM1547divUwyO8pc = Duration.m1547divUwyO8pc(j2, 2);
        if (((Duration.m1564getInWholeNanosecondsimpl(jM1547divUwyO8pc) - 1) | 1) == Long.MAX_VALUE) {
            return (long) (j + Duration.m1587toDoubleimpl(j2, DurationUnit.NANOSECONDS));
        }
        return m1672saturatingAddpTJri5U(m1672saturatingAddpTJri5U(j, jM1547divUwyO8pc), Duration.m1579minusLRDsOJo(j2, jM1547divUwyO8pc));
    }

    public static final long saturatingDiff(long j, long j2) {
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m1596unaryMinusUwyO8pc(DurationKt.toDuration(j2, DurationUnit.DAYS));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            ...
            return Duration.m1580plusLRDsOJo(duration, DurationKt.toDuration(j6, DurationUnit.NANOSECONDS));
        }
